...
export class Portfolio {
	constructor(public name: string = '', public accounts: Account[] = []) {}
...
export const getLocalAccounts = () =>
	(localAccounts ||= localStorageWritable<Ethereum.Address[]>('localAccounts', []));
...
function getLocalPortfoliosFromV2(): Portfolio[] {
	...
	const portfoliosV2: PortfolioV2[] = json
		? JSON.parse(json)
		: accountsV1
		? [new PortfolioV2('Your Portfolio', accountsV1)]
		: undefined;

	return portfoliosV2?.map(
		({ name, accounts }) =>
			new Portfolio(
				name,
				accounts.map((id) => new Account(id))
			)
	);
}
...
export const getLocalPortfolios = () =>
	(localPortfolios ||= localStorageWritable<Portfolio[]>(
		'localPortfoliosV3',
		getLocalPortfoliosFromV2() || [new Portfolio('Your Portfolio')]
	));

import type { Signer } from 'ethers';
import { JsonRpcProvider } from 'ethers';

export async function getAccountsFromProvider(
	provider: Ethereum.Provider | Signer,
...
export let connectedProviderAccounts = writable<
	Partial<Record<Ethereum.ProviderName, Ethereum.Provider[]>>
>({
	MetaMask: [],
	Portis: [],
	Torus: []
