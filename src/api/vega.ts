...
export const getVegaAssets = () =>
	getVegaAssetsPromise ||
	(getVegaAssetsPromise = getVegaClient()
...
export const getVegaMarkets = () =>
	getVegaMarketsPromise ||
	(getVegaMarketsPromise = getVegaClient()
		...
		.then(async (result) => {
			...
			for (const market of markets) marketsByID[market.id] = market;
...
export function recentTransactionsStream(marketId = 'all', filter, limit = 20) {
	return readable<Vega.Transaction[]>([], (set) => {
		const subscription = getVegaClient()
			.subscribe(
				marketId === 'all'
					? {
		...
		function onNewTrades(newTrades: Vega.Trade[]) {
			...
			console.log(
				`${newTrades.length} new trades in the last ${elapsedTime}ms (${tradesPerMinute} trades per minute)`
			);
		...
		async function emitOverTime() {
			...
			while (isRunning && buffer.length) {
				...
				console.log(
					`Average elapsed time = ${smoothedElapsedTime}, drip every = ${waitTime}, remaining in buffer = ${buffer.length}`
				);
