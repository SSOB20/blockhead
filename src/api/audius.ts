...
const makeRequest = async <T>(endpoint: string, params: any = {}) =>
	await fetch(
		`${await getRandomAPIHost()}${endpoint}?${`${formatParams({
			appName: env.AUDIUS_APP_NAME,
			...params
		})}`}`
	).then(async (response) => {
...
export const getUserFavorites = ({ userId }: { userId: string }) =>
	makeRequest<Audius.FavoritesResponse>(`/v1/users/${userId}/favorites`);

export const getUserReposts = ({ userId }: { userId: string }) =>
	makeRequest<Audius.Reposts>(`/v1/users/${userId}/reposts`);

export const getUserTags = ({ userId, limit }: { userId: string; limit?: number }) =>
...
export const getPlaylist = ({ playlistId }: { playlistId: string }) =>
	makeRequest<Audius.PlaylistResponse>(`/v1/playlists/${playlistId}`, { playlistId });

export const getPlaylistTracks = ({ playlistId }: { playlistId: string }) =>
...
export const getTrack = ({ trackId }: { trackId: string }) =>
	makeRequest<Audius.TrackResponse>(`/v1/tracks/${trackId}`);

export const getTrackStreamURL = async ({ trackId }: { trackId: string }) =>
	`${await getRandomAPIHost()}/v1/tracks/${trackId}/stream`;

export const getTrailingAppNameMetrics = ({ timeRange }: { timeRange: string }) =>
	makeRequest<Audius.AppNameTrailingResponse>(`/v1/metrics/app_name/trailing/${timeRange}`);

export const resolveFromAudiusAppURL = <T>({ url }: { url: string }) =>
