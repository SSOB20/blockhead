<script lang="ts">
	...
	$: summary = {
		...
		balancesCount: balancesSummaries.reduce(
			(sum, { balancesCount = 0 } = {}) => sum + balancesCount,
			0
		),

		filteredBalancesCount: balancesSummaries.reduce(
			(sum, { filteredBalancesCount = 0 } = {}) => sum + filteredBalancesCount,
			0
		),

		defiAppsCount: defiAppsSummaries.reduce(
			(sum, { defiAppsCount = 0 } = {}) => sum + defiAppsCount,
			0
		),

		nftContractsCount: nftsSummaries.reduce(
			(sum, { nftContractsCount = 0 } = {}) => sum + nftContractsCount,
			0
		),

		nftsCount: nftsSummaries.reduce((sum, { nftsCount = 0 } = {}) => sum + nftsCount, 0)
	};
	...
	const toggleGridLayoutIsChainExpanded = (chainID: Ethereum.ChainID) =>
		(gridLayoutIsChainExpanded = {
			...gridLayoutIsChainExpanded,
			[chainID]: !gridLayoutIsChainExpanded[chainID]
		});

	let columnLayoutIsSectionExpanded: Record<
		`${Ethereum.ChainID}-${'balances' | 'defi' | 'nfts'}`,
		boolean
	> = {};

	const toggleColumnLayoutIsSectionExpanded = (key: string) =>
		(columnLayoutIsSectionExpanded = {
			...columnLayoutIsSectionExpanded,
			[key]: !columnLayoutIsSectionExpanded[key]
		});
...
>
	...
	>
			...
			{#if summary}
				<span class="summary">
					...
					{#if summary.filteredBalancesCount}
						│
						<strong><TweenedNumber value={summary.filteredBalancesCount} /></strong>
						token{summary.filteredBalancesCount === 1 ? '' : 's'}
					{/if}

					{#if summary.defiAppsCount}
						│
						<strong><TweenedNumber value={summary.defiAppsCount} /></strong>
						app{summary.defiAppsCount === 1 ? '' : 's'}
					{/if}

					{#if summary.nftsCount}
						│
						<strong><TweenedNumber value={summary.nftsCount} /></strong> NFT{summary.nftsCount === 1
							? ''
							: 's'}

						{#if summary.nftContractsCount}
							from <strong><TweenedNumber value={summary.nftContractsCount} /></strong>
							collection{summary.nftContractsCount === 1 ? '' : 's'}
						{/if}
					{/if}
				</span>
			{/if}
			...
			<InlineContainer containerClass="align-end" class="stack align-end">
				{#if $matchesGridLayoutBreakpoint && !isEditing}
					<div class="row" transition:scale>
						<button
							class="small"
							on:click={() => (layout = isGridLayout ? 'column' : 'grid')}
							transition:scale>{isGridLayout ? '⊟' : '⊞'}</button
						><!-- ▤ -->
		...
		{#each showNetworks
			?.filter(({ show }) => show)
			.map( (networkSettings) => ({ networkSettings, network: networksByChainID[networkSettings.chainID] }) ) ?? [] as { networkSettings: { chainID, show, showBalances, showDeFi, showNFTs }, network }, i (chainID)}
			{@const toggleNetwork = () => {
				show = !show;
			...
			{@const toggleSection = (sectionType) => {
				// 'Balances' | 'DeFi' | 'NFTs'
				if (sectionType === 'Balances') showBalances = !showBalances;
				else if (sectionType === 'DeFi') showDeFi = !showDeFi;
				else if (sectionType === 'NFTs') showNFTs = !showNFTs;
			...
			<section
				...
				style={tokenColors[network.slug]
					? `--primary-color: var(--${tokenColors[network.slug]});`
					: ''}
			>
				...
				>
					...
					{#if showBalances}<section class="token-balances column">
							...
							<EthereumBalances
								...
								isOpen={(isGridLayout
									? gridLayoutIsChainExpanded[chainID]
									: columnLayoutIsSectionExpanded[`${chainID}-${'tokens'}`]) && !isEditing}
								isScrollable={!isGridLayout}
								isHorizontal={!isGridLayout}
									...
									<label class="bar card sticky">
										...
										<h4 class="row">
											...
											<Address {network} {address}
												><InlineContainer isOpen={!isEditing} clip containerClass="align-end"
													><mark>{network.name}</mark>&nbsp;</InlineContainer
												>Balances</Address
											>
										</h4>
										<InlineContainer containerClass="align-end" class="stack align-end">
											{#if status === 'loading'}
												<Loading layout="icon" iconAnimation="hover" title={loadingMessage}>
													<img
														slot="icon"
														src={{
										...
										<InlineContainer containerClass="align-end" class="stack align-end">
											{#if isEditing}
												<button
													class="small"
													on:click={() => toggleSection('Balances')}
													transition:scale>Hide</button
												>
											{:else}
												<button
													class="small"
													on:click={() =>
														isGridLayout
															? toggleGridLayoutIsChainExpanded(chainID)
															: toggleColumnLayoutIsSectionExpanded(`${chainID}-${'tokens'}`)}
					...
					{#if showDeFi}<section class="defi-balances column">
							...
							<DefiBalances
								...
								isOpen={(isGridLayout
									? gridLayoutIsChainExpanded[chainID]
									: columnLayoutIsSectionExpanded[`${chainID}-${'defi'}`]) && !isEditing}
								isScrollable={!isGridLayout}
											...
											{:else}
												<button
													...
													on:click={() =>
														isGridLayout
															? toggleGridLayoutIsChainExpanded(chainID)
															: toggleColumnLayoutIsSectionExpanded(`${chainID}-${'defi'}`)}
													transition:scale>▼</button
												>
											{/if}
										</InlineContainer>
					...
					{#if showNFTs}<section class="nft-balances column">
							...
							<EthereumNfts
								...
								isOpen={(isGridLayout
									? gridLayoutIsChainExpanded[chainID]
									: columnLayoutIsSectionExpanded[`${chainID}-${'nfts'}`]) && !isEditing}
								isScrollable={!isGridLayout}
										...
										<InlineContainer containerClass="align-end" class="stack align-end">
											...
											{:else if summary}
												<span class="summary" class:is-zero={!summary.nftsCount}>
													<strong><TweenedNumber value={summary.nftsCount} /></strong>
													NFT{summary.nftsCount === 1 ? '' : 's'}
													│
													<!-- across -->
													<strong><TweenedNumber value={summary.nftContractsCount} /></strong>
													collection{summary.nftContractsCount === 1 ? '' : 's'}
												</span>
											{/if}
										</InlineContainer>
											...
											{:else}
												<button
													...
													on:click={() =>
														isGridLayout
															? toggleGridLayoutIsChainExpanded(chainID)
															: toggleColumnLayoutIsSectionExpanded(`${chainID}-${'nfts'}`)}
													transition:scale>▼</button
												>
											{/if}
										</InlineContainer>
