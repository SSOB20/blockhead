...
{#if user}
	<div class="audius-user card layout-{layout}">
		{#if user.cover_photo}
			<img
				class="cover-image card-background"
				srcset={Object.entries(user.cover_photo)
			...
			{#if user.profile_picture}
				<img
					...
					srcset={user.handle === 'Audius'
						...
						: Object.entries(user.profile_picture)
								.map(([size, src]) => `${src}`)
								.join(', ')}
			...
			<div class="column">
				<div class="bar">
					...
					<p class="card-annotation">
						Audius {user.track_count || user.album_count
							? `${user.is_verified ? 'Verified ' : ''}Artist`
							: user.playlist_count
							? 'Curator'
							: user.repost_count
							? 'Influencer'
