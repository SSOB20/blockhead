<script lang="ts">
	...
	$: filteredBalances = balances
		.filter(
			({ type, value, token, balance }) =>
				type !== 'nft' &&
				((showNativeCurrency && tokensAreEqual(network.nativeCurrency, token)) ||
					!(
						!showSmallValues &&
						// type === 'dust' ||
		...
		.sort(
			sortBy === 'value-descending'
				? (a, b) => b.value - a.value || a.balance - b.balance
				: sortBy === 'value-ascending'
				? (a, b) => a.value - b.value || b.balance - a.balance
				: sortBy === 'ticker-ascending'
				? (a, b) => a.token.symbol?.localeCompare(b.token.symbol)
				: undefined
		);
	...
	const tokensAreEqual = (token1, token2) =>
		...
		token1.symbol === token2.symbol && token1.decimals === token2.decimals;
...
{#if address}
	...
	>
		...
		<div class:scrollable-list={isScrollable && filteredBalances.length > 45}>
			<div
				class="ethereum-balances card"
				class:horizontal={isHorizontal}
				class:show-amounts-and-values={tokenBalanceFormat === 'both'}
			>
				{#each filteredBalances as { type, token, balance, value, rate }, i (token.address || token.symbol || token.name)}
					<span
						...
						in:scale
						animate:flip={{
							duration: filteredBalances.length < 50 ? 500 : 0,
							delay: (300 * i) / filteredBalances.length,
							easing: quintOut
						}}
					>
...
<style>
	...
	.ethereum-balance.is-selectable:hover,
	.ethereum-balance.is-selectable:focus {
