...
<p class="transaction row-inline" title="Transaction {transactionID}">
	...
	{/if}
	{#if transactionID && showTransactionID}<span class="transaction-id"
			><EthereumTransactionID {network} {transactionID} format="middle-truncated" /></span
		>
	{/if}
	<span>
		{#if transactionID && showTransactionID && transactionIndex !== undefined}<span class="muted"
				>(</span
			>{/if}{#if transactionIndex !== undefined}<a
				class="muted"
				href={transactionID && !showTransactionID
					? `/explorer/${network.slug}/${transactionID}`
					: undefined}>#{transactionIndex}</a
			>
		{/if}{#if blockNumber !== undefined}
			<span class="muted">in block</span>
			<EthereumBlockNumber
				{network}
				{blockNumber}
				format="number-only"
			/>{#if transactionID && showTransactionID && transactionIndex !== undefined}<span
					class="muted">)</span
				>{/if}
		{/if}
	</span>
