<script lang="ts">
	...
	$: if (zapperDefiProtocolBalances)
		quoteTotal =
			zapperDefiProtocolBalances.reduce(
				(sum, { meta }) =>
					sum + Number(meta?.find(({ label, type, value }) => label === 'Total')?.value ?? 0),
				0
			) * zapperFiatRate;
	...
	$: computedLayout =
		layout === 'auto'
			? showUnderlyingAssets
				? 'vertical'
				: 'horizontal' // 'horizontal-alternate'
			: layout;
...
{#if web3AppConfig}
	<div class="column defi-app-views">
		{#each web3AppConfig.views as { name, slug, links, chainId, colors, erc20Tokens, nfts, contracts, providers, embeds }}
			{@const totalViewItems =
				(erc20Tokens?.length ?? 0) +
				(nfts?.length ?? 0) +
				(contracts?.length ?? 0) +
				(providers && Object.entries(providers).length)}
			...
			>
				...
				>
					<svelte:fragment slot="header">
						<div class="bar">
							<span class="row-inline">
								...
								{#if _links?.length}
									<span>
										({#each _links as href, i}<a {href} target="_blank"
												>{new URL(href)?.host.replace(/^www[.]/, '')}</a
											>{#if i < _links.length - 1}{', '}{/if}{/each})
									</span>
								{/if}
							</span>
					...
					<div class="column defi-app-view-items">
						...
						{#if !address}
							...
							{#each contracts ?? [] as { name, address: contractAddress }}
								<EthereumAccountOrContract {network} addressOrEnsName={contractAddress} {provider}>
								...
								<div class="card">
									...
									>
										...
										<div
											...
											class:scrollable-list={defiProtocolBalances.flatMap(
												({ products }) => products
											).length > 6}
										>
											{#each defiProtocolBalances as { appId, products, meta }, i}
												{#each products as { label, assets, meta: productMeta }, j (label)}
													...
													>
														...
														<div class="bar">
															<h5
																class:card-annotation={computedLayout === 'horizontal-alternate'}
																title={label}
															>
																<a
																	href="/apps/{web3AppsByProviderName.zapper?.[appId]?.slug ??
																		appId}/address/{address}">{label}</a
																>
															</h5>
														...
														<div class="defi-protocol-balances column">
															{#each assets as { type, category, label, symbol, tokenAddress, img, decimals, address, balance, balanceUSD, balanceRaw, price, protocol, protocolDisplay, protocolSymbol, protocolImg, tokens, apy, reserve, share, supply }}
																<div class="defi-protocol-balance column">
																	<div class="bar">
																		<TokenBalanceWithConversion
																			...
																			balance={balanceRaw && Number.isInteger(Number(balanceRaw))
																		...
																		{:else if label && label !== symbol}
																			<span class="card-annotation">{label}</span>
																		{:else}
																			<span class="card-annotation"
																				>{formatKebabCase(type)}{category && type !== category
																					? ` ${category}`
																					: ''}</span
																			>
																		{/if}
																	...
																	{#if showUnderlyingAssets && tokens?.length}
																		<div class="underlying">
																			{#each tokens as { address, decimals, symbol, tokenImageUrl, balance, balanceUSD, balanceRaw, price, reserve, weight, isCToken }}
																				<abbr class="underlying-asset" in:scaleFont title={label}>
																					...
																					<TokenBalanceWithConversion
																						...
																						icon={tokenImageUrl}
																						balance={balanceRaw &&
																						Number.isInteger(Number(balanceRaw))
																							? formatUnits(balanceRaw, decimals)
																							: balance}
																						convertedValue={balanceUSD * zapperFiatRate}
																						conversionCurrency={zapperQuoteCurrency}
								...
								<div class="card">
									...
									>
										...
										<div class="defi-balances column">
											{#each defiBalances as { adapterBalances, metadata }, i (metadata.name + i)}
												<div
													class="card defi-protocol layout-{computedLayout}"
													transition:scaleFont|local
													animate:flip={{ duration: 300, delay: Math.abs(i) * 10 }}
												>
													<!-- style={cardStyle(colors || web3AppConfig.colors)} -->
													<h5
														class:card-annotation={computedLayout === 'horizontal-alternate'}
													...
													>
														<img
															class="card-background"
															src={`https://${metadata.iconURL}`}
															alt={metadata.name}
															width="20"
														/>
														<a
															href="/apps/{web3AppsByProviderName.zerionDefiSDK?.[metadata.name]
																?.slug}/address/{address}">{metadata.name}</a
														>
													</h5>
																				...
																				<p in:scaleFont>
																					...
																					<TokenBalance
																						...
																						balance={formatUnits(
																							...
																							underlyingBalance.metadata.decimals
																						)}
																						isDebt={adapterBalance.metadata.adapterType === 'Debt'}
																					/>
																				</p>
						...
						{#if erc20Tokens && !(address && ((providers?.zapper && defiProvider === 'Zapper') || (providers?.zerionDefiSDK && defiProvider === 'Zerion DeFi SDK')))}
										...
										{#if !address}
											<div class="column" transition:scale>
												...
												>
													...
													<h4 slot="title">Current Price</h4>
												</CurrentPrice>
												...
												<EthereumContractExplorer {network} {address} {provider} />
											</div>
										{:else}
											<div class="card" transition:scale>
												...
												>
													<svelte:fragment slot="header">
														<div class="bar">
															...
															<div class="card-annotation">
																{$preferences.tokenBalancesProvider}
															</div>
														</div>
													...
													{#each [balances?.find((balance) => balance.token.address.toLowerCase() === erc20Token.address.toLowerCase())].filter((_) => _) as { type, token, balance, value, rate }, i (i)}
														<TokenBalanceWithConversion
															{tokenBalanceFormat}
															{network}
															erc20Token={token}
						...
						{#each nfts ?? [] as { address: contractAddress }}
							{#if !address}
								<section class="card">
									<EthereumContractExplorer {network} address={contractAddress} {provider} />
								</section>
							{:else}
								<!-- <section class="card">
									<EthereumNfts
				...
				{#if providers?.theGraph && !address}
					<div class="card" id="subgraph">
						<div class="bar">
							<h4>
								<a
									href="https://thegraph.com/explorer/subgraph/{providers.theGraph.match(
										/[^/]+\/[^/]+$/
									)}">Subgraph ({providers.theGraph.match(/[^/]+\/[^/]+$/)})</a
								>
							</h4>
...
{#if address}
	<div class="card bar options" transition:scale>
		<div class="row">
			<h3>Show</h3>
			<label>
				<TokenBalanceFormatSelect bind:tokenBalanceFormat {quoteCurrency} />
			</label>
			<label>
