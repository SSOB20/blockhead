...
<section class="column">
	<header class="bar">
		...
		<label>
			<span
				>{selectedMethod
					? isReadable(selectedMethod)
						? 'Query'
						: isWritable(selectedMethod)
						? 'Action'
						: 'Method'
					: 'Method'}</span
			>
	...
	{#if selectedMethod}
		...
		{@const args = method.inputs.map(
			(input, i) => inputValues[`${method.name || i}/${input.name}`]
		)}
		...
		<TransactionFlow
			...
			{network}
			getContract={({ signer }) =>
				new Contract(contractAddress, abi, isWritable(selectedMethod) ? signer : provider)}
		...
		>
				...
				{#if method.inputs.length}
					...
					<HeightContainer>
						{#each method.inputs as input, i (`${method.name || i}/${input.name}`)}
							...
							<label
								class="input-param"
								transition:scale={{ duration: 300, start: 0.8, delay: i * 10 }}
								animate:flip
							>
								<span>
									{#if input.name}
								...
								{#if input.type === 'address'}
									<AddressInput bind:address={inputValues[inputKey]} required />
								...
								{:else if input.type.startsWith('uint')}
									...
									/>
								{:else}
									<input type="text" required bind:value={inputValues[inputKey]} />
								{/if}

								<abbr
									class="card-annotation"
									title="{input.type} ({input.indexed ? `indexed ` : ''}{input.internalType})"
									>{input.type}</abbr
								>
							</label>
						{/each}
				...
				<div class="row spaced">
					...
					<div class="row">
						{#if isReadable(method)}
							<button type="button" class="medium" on:click={actions.query}
								><img
									src={networkProviderConfigByProvider[$preferences.rpcNetwork]?.icon}
									width="16"
								/> Query Contract ›</button
							>
						{:else if isWritable(method)}
							<button type="button" class="tenderly medium" on:click={actions.simulate}
								><img src={TenderlyIcon} width="16" /> Simulate Transaction ›</button
							>

							<button type="button" class="medium" on:click={actions.sign}
								><img src={walletIcon} width="16" /> Sign & Broadcast Transaction ›</button
							>
						{:else}{/if}
					</div>
				</div>
			...
			<svelte:fragment slot="simulating-actions" let:actions>
				<div class="row spaced">
					...
					<div class="row">
						<button type="button" class="medium destructive" on:click={actions.cancel}
							>« Start Over</button
						>
						<button type="button" class="tenderly medium" on:click={actions.simulate}
							><img src={TenderlyIcon} width="16" /> Simulate Again</button
						>
					</div>
				</div>
			</svelte:fragment>
			...
			<svelte:fragment slot="pending-message" let:network>Pending...</svelte:fragment>

			<!-- <svelte:fragment slot="failed"> -->

			<!-- <svelte:fragment slot="success"> -->
			<svelte:fragment slot="success-message" let:network>Success!</svelte:fragment>
		</TransactionFlow>
	{/if}
</section>
