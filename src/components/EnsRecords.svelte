<script lang="ts">
	...
	const descriptions = Object.fromEntries([
		[
			'contentHash',
			`contenthash is used to store hashes for IPFS, Skynet, and Swarm, and Tor .onion addresses, which permit resolving ENS addresses to distributed content (eg, websites) hosted on these distributed networks.\n\nSource: https://docs.ens.domains/dapp-developer-guide/resolving-names#looking-up-other-resources`
		],
		...Object.entries(eip634GlobalKeys).map(([k, v]) => [
			k,
			`${v}\n\nSource: https://eips.ethereum.org/EIPS/eip-634#global-keys`
		]),
		...Object.entries(eip634ServiceKeys).map(([k, v]) => [
			k,
			...
			<section class="card column">
				<header class="bar">
					...
					<a
						...
						href="https://eips.ethereum.org/EIPS/eip-634"
						target="_blank"
						rel="noreferrer"><abbr>EIP-634</abbr></a
					>
				</header>

				<hr />

				{#each [...orderedTextRecordKeys
						.filter((key) => textRecords.has(key))
						.map((key) => [key, textRecords.get(key)]), ...[...textRecords.entries()]
						.filter(([key]) => !orderedTextRecordKeys.includes(key))
						.sort( (a, b) => a[0].localeCompare(b[0]) )].sort( (a, b) => (a[1] === null ? 1 : b[1] === null ? -1 : 0) ) as [key, value], i (key)}
					<dl transition:fade>
						<!-- animate:flip|local={{duration: 300, delay: i * 10}} -->
						<dt>
							{#if descriptions[key]}
						...
						<dd>
							...
							{:else if key === 'url' || key === 'website'}
								<output
									><a href={value} target="_blank" rel="noreferrer" referrerpolicy="no-referrer"
										>{value}</a
									></output
								>
							...
							{:else if key === 'com.github'}
								<output
									><a href="https://github.com/{value}" rel="noreferrer" target="_blank">{value}</a
									></output
								>
							{:else if key === 'com.linkedin'}
								<output
									><a href="https://www.linkedin.com/in/{value}" rel="noreferrer" target="_blank"
										>{value}</a
									></output
								>
							{:else if key === 'com.reddit'}
								<output
									><a href="https://reddit.com/u/{value}" rel="noreferrer" target="_blank"
										>{value}</a
									></output
								>
							{:else if key === 'org.telegram'}
								<output
									><a href="https://t.me/{value}" rel="noreferrer" target="_blank">{value}</a
									></output
								>
							{:else if key === 'com.twitter'}
								<output
									><a href="https://twitter.com/{value}" rel="noreferrer" target="_blank">{value}</a
									></output
								>
		...
		{#if contentHash}
			<div class="card column">
				<header class="bar">
					...
					<a
						class="card-annotation"
						href="https://eips.ethereum.org/EIPS/eip-1577"
						target="_blank"
						rel="noreferrer"><abbr>EIP-1577</abbr></a
					>
				</header>
		...
		{#if cryptoAddressRecords?.size}
			<div class="card column">
				<header class="bar">
					...
					<a
						class="card-annotation"
						href="https://eips.ethereum.org/EIPS/eip-2304"
						target="_blank"
						rel="noreferrer"><abbr>EIP-2304</abbr></a
					>
				</header>

				<hr />

				{#each [...cryptoAddressRecords.entries()].sort( (a, b) => (a[1] === null ? 1 : b[1] === null ? -1 : 0) ) as [key, address], i (key)}
					<dl transition:fade|local>
						<!-- animate:flip|local={{duration: 300, delay: i * 10}} -->
						<dt>
							<TokenIcon
								{network}
								symbol={networksBySlip44[key]?.symbol || chainsBySlip44[key]?.symbol}
							/>
							<abbr title={networksBySlip44[key]?.name || chainsBySlip44[key]?.name || ''}
								>{networksBySlip44[key]?.symbol || chainsBySlip44[key]?.symbol}</abbr
							>
						</dt>
						<dd>
							{#if address === null}
