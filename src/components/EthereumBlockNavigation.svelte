	...
	<nav
		...
		on:click={(e) => {
			...
			[$networkSlug, $query] = [
				network.slug,
				Math.round(
					adjustedX * Math.max(blockNumber ?? -Infinity, latestBlockNumber ?? -Infinity)
				).toString()
			];
		}}
		transition:fade
	...
	>
		...
		{#if blockNumber !== 0 && !(showBeforeAndAfter && blockNumberBefore === 0)}
			<span class="first-block" style="--block-number: {0}" transition:scaleFont>
				<span title="{network.name} Genesis Block"
					><EthereumBlockNumber {network} blockNumber={0} /></span
				>
				<span>«</span>
				<!-- <span>🔗</span> -->
			</span>
		...
		{#if showBeforeAndAfter && blockNumber !== 0}
			<span class="previous-block" style="--block-number: {blockNumberBefore}" transition:scaleFont>
				<span title="Previous {network.name} Block"
					><EthereumBlockNumber {network} blockNumber={blockNumberBefore} /></span
				>
				<span>‹</span>
				<!-- <span>🔗</span> -->
			</span>
		...
		<span
			class="current-block"
			style="--block-number: {blockNumber ?? latestBlockNumber
				? Math.ceil(latestBlockNumber / 2)
				: 1}"
			transition:scaleFont
		>
			<span title="Current {network.name} Block"
				><EthereumBlockNumber {network} {blockNumber} /></span
			>
		</span>
		...
		{#if showBeforeAndAfter && blockNumber !== latestBlockNumber}
			<span class="next-block" style="--block-number: {blockNumberAfter}" transition:scaleFont>
				<!-- <span>🔗</span> -->
				<span>›</span>
				<span title="Next {network.name} Block"
					><EthereumBlockNumber {network} blockNumber={blockNumberAfter} /></span
				>
			</span>
		{/if}
		...
		{#if blockNumber !== latestBlockNumber && !(showBeforeAndAfter && blockNumberAfter === latestBlockNumber)}
			<span
				class="latest-block"
				style="--block-number: {latestBlockNumber ?? blockNumber ? blockNumber * 2 : 2}"
				transition:scaleFont
			>
				<!-- <span>🔗</span> -->
				<span>»</span>
				<span title="Latest {network.name} Block"
					><EthereumBlockNumber {network} blockNumber={latestBlockNumber} /></span
				>
			</span>
		{/if}
...
<style>
	...
	.latest-block {
		...
		/* margin-left: auto; */
		flex-grow: max(
			1 - var(--current-block-number) / var(--latest-block-number),
			1 - var(--latest-block-number) / max(var(--current-block-number), 1)
		);
		justify-content: end;
	}
