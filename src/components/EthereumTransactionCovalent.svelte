	...
	const convertCovalentTransaction = ({
		...
		({
			...
			logEvents: log_events
				?.map(
					({
						...
						decoded: decoded && {
							...
							params:
								decoded.params?.map(({ name, type, value, indexed, decoded }) => ({
									name,
									type,
				...
				.sort(
					(logEvent1, logEvent2) => logEvent1.indexInTransaction - logEvent2.indexInTransaction
				),
...
{#if network && (transaction || erc20TokenTransaction || erc20TokenTransfer)}
	...
	>
		...
		{#if !isSummary && logEvents?.length}
			...
			{/if}
			<div
				class="log-events column"
				class:scrollable-list={isExhaustive && logEvents.length > (isStandaloneLayout ? 8 : 16)}
			>
				<!-- transition:fade|local -->
