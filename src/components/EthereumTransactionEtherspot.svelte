<script lang="ts">
	...
	const _formatUnits = (value, decimals) => {
		try {
			return value
				? formatUnits(typeof value === 'string' ? value.replace(/\.0*$/, '') : value, decimals)
				: 0;
		} catch (e) {
			// console.error(e)
			return value;
	...
	const convertEtherspotTransaction = (transaction: EtherspotTranaction) =>
		({
			...
			transferredToken:
				transaction.asset?.category === TransactionAssetCategories.Token
					? {
	...
	$: ({
		...
		transferredToken
	} = (transaction ? convertEtherspotTransaction(transaction) : {}) as Transaction);
	...
	$: contextIsSender =
		contextualAddress &&
		fromAddress &&
		contextualAddress.toLowerCase() === fromAddress.toLowerCase();
	$: contextIsReceiver =
		contextualAddress && toAddress && contextualAddress.toLowerCase() === toAddress.toLowerCase();
...
{#if network && transaction}
	<div
		class="transaction layout-{layout} column"
		class:card={isStandaloneLayout}
		class:unsuccessful={!isSuccessful}
			...
			{#if isStandaloneLayout}
				...
				<div class="log-events column">
					{#each logEvents as logEvent}
						<div class="card">
							<EthereumLogEventEtherspot {network} {logEvent} {detailLevel} {contextualAddress} />
						</div>
					{/each}
				</div>
			{:else}
				<div class="log-events column" class:scrollable-list={logEvents.length > 16}>
					<!-- transition:fade|local -->
					{#each logEvents as logEvent}
						<EthereumLogEventEtherspot {network} {logEvent} {detailLevel} {contextualAddress} />
					{/each}
