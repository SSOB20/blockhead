<script lang="ts">
	...
	$: lastUpdate = updatesByNetwork
		.get(network)
		?.find((upgrade) => block.blockNumber >= upgrade.blockNumber);
...
{#key network}
	...
	<div class="consensus card column">
		<div class="row">
			{#if lastUpdate?.consensus.type === 'Proof of Work'}
				<span class="miner">
					<abbr
						title={'A network node that finds valid proof-of-work for new blocks, by repeated pass hashing (Ethash).\n\nSource: https://ethereum.org/en/glossary/#miner'}
						>Miner</abbr
					>

					{#if block.minerAddress}
						<span class="miner-address">
							<Address {network} address={block.minerAddress} format="middle-truncated" />
						</span>
					{/if}
				</span>
				...
				<span>
					discovered <abbr
						title={'The random value in a block that was used to satisfy the proof-of-work.\n\nSource: https://ethereum.org/en/glossary/#nonce'}
						>nonce</abbr
					>
				...
				<span>
					at <abbr
						title={'A network-wide setting that controls how much computation is required to produce a proof-of-work.\n\nSource: https://ethereum.org/en/glossary/#difficulty'}
						>difficulty</abbr
					>
			...
			{:else if lastUpdate?.consensus.type === 'Proof of Stake'}
				<span class="validator">
					<abbr
						title={'A node in a proof-of-stake system responsible for storing data, processing transactions, and adding new blocks to the blockchain.\n\nSource: https://ethereum.org/en/glossary/#validator'}
						>Validator</abbr
					>
			...
			<span>
				producing hash
				<output class="block-hash"
					><abbr title="Block hash: {block.hash}"
						>{formatTransactionHash(block.hash, 'middle-truncated')}</abbr
					></output
				>
			</span>
		</div>
		...
		<div class="including row">
			...
			<span class="extra-data-container">
				extra data
				<output class="extra-data"
					><abbr title={`Extra data:\n${block.extraData}`}>{_toUtf8String(block.extraData)}</abbr
					></output
				>
			</span>
		</div>
	...
	{#if block.transactions?.length}
		...
		<div class="bar">
			<div class="row">
				<h3>
					Transactions
					{#if status === 'resolved'}({block.transactions.length}{block.transactions.length === 100
							? '+'
							: ''}){/if}
				</h3>

				{#if showTransactions}
					<button class="small" on:click={() => (showTransactions = false)} transition:scale
						>Hide</button
					>
				{/if}
			</div>
			...
			{:else}
				<button class="small" on:click={() => (showTransactions = true)} transition:scale
					>Show</button
				>
			{/if}
	...
	<div class="footer bar">
		<span class="gas-stats">
			<abbr
				title={'Gas Used\n\nGas: A virtual fuel used in Ethereum to execute smart contracts. The EVM uses an accounting mechanism to measure the consumption of gas and limit the consumption of computing resources.\n\nSource: https://ethereum.org/en/glossary/#gas'}
				>{formatNumber(block.gasUsed)}</abbr
			>
			/
			<abbr
				title={'Gas Limit\n\nThe maximum amount of gas a transaction or block may consume.\n\nSource: https://ethereum.org/en/glossary/#gas-limit'}
				>{formatNumber(block.gasLimit)}</abbr
			>
			computational gas units used ({formatPercent(block.gasUsed / block.gasLimit)})
		</span>
