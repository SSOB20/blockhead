...
>
	{#if tokenBalanceFormat === 'original' || tokenBalanceFormat === 'both'}
		<span class="balance" transition:scaleFont|local
			><!-- style="font-size: {sizeByVolume(convertedValue)}em" -->
			<TokenBalance
				{network}
				{symbol}
				{address}
	...
	{#if tokenBalanceFormat === 'converted' || tokenBalanceFormat === 'both'}
		<span class="balance-converted" transition:scaleFont|local={{ delay: 50 + animationDelay }}>
			{#if tokenBalanceFormat === 'both'}{#if showParentheses}({/if}{/if}<TokenBalance
				{network}
				symbol={conversionCurrency}
				balance="{convertedValue}{showDecimalPlaces}"
				showPlainFiat={true}
				{isDebt}
				{tween}
				{clip}
			...
			{/if}{#if showConversionRate && conversionRate}<span class="rate">
					at <TokenRate
						rate={conversionRate}
						quoteToken={conversionCurrency}
						baseToken={symbol}
						layout="horizontal"
					/></span
				>{/if}{#if tokenBalanceFormat === 'both' && showParentheses}){/if}
		</span>
	{/if}
</span>
