	...
	<section>
		<EthereumContractMetadataLoader
			...
			whenLoaded={async (contractMetadata) => {
				...
				const targetSourcePath = Object.keys(contractMetadata.sources).find((sourcePath) =>
					sourcePath.match(new RegExp(`(?:^|[/])${contractName}[.]`))
				);

				if (targetSourcePath) showContractSourcePath = targetSourcePath;
			}}
		...
		>
			<header class="bar" slot="header" let:contractMetadata>
				<slot name="title" contractName={name} {network} {address}>
					<h4>
						<Address {network} {address} format="middle-truncated" let:formattedAddress
			...
			<div class="stack">
				{#key showContractSourcePath}
					{#if contractMetadata && showContractSourcePath !== 'EVM Bytecode'}
						...
						{@const solidityDefinitionType =
							...
							source.content?.match(
								new RegExp(
									`((?:abstract )?library|contract|interface|function|constant|struct|enum|type|error)\\s+(${sourceFileName})`
								)
							)?.[1]}
						...
						>
							<header class="bar">
								...
								>
									<h4>{sourceFile}</h4>
									{#if source.license}<small
											><span class="card-annotation">{source.license}</span></small
										>{/if}
								</abbr>

								<abbr
									class="card-annotation"
									title="{contractMetadata?.language} {contractMetadata?.compiler?.version}"
								>
							...
							{:else if source.urls?.length}
								{@const ipfsCid = source.urls
									.find((url) => url.includes('dweb:'))
									?.match(/^dweb:\/ipfs\/(.+)$/)?.[1]}

								<IpfsLoader contentId={ipfsCid} format="text" let:content={sourceCode} let:ipfsUrl>
					...
					{:else}
						<code class="card scrollable-list" style="height: 7.5em" transition:fade
							>{contractCode}</code
						>
					{/if}
				{/key}
