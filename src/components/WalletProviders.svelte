<script lang="ts">
	...
	import {
		getWalletProvider,
		getWalletProviderAndInstance
	} from '../data/ethereum/wallets_/wallets';
	...
	const load3IDConnect = async (account: Account) => {
		...
		const { getDIDProviderFromEthereumProvider, getDID, authenticateDID } = await import(
			'../data/ceramic/3id'
		);
...
<section class="wallet-providers column">
	<div class="metamask column">
		<Loader
			...
			let:load
			fromPromise={() =>
				loadMetaMaskProvider().then((provider) => getAccountsFromProvider(provider, 'MetaMask'))}
		...
		>
			...
			<Portfolio name="MetaMask Wallet" provider={portfolioProvider} {accounts}>
				<h1 slot="title">
					<img src={MetaMaskIcon} alt="MetaMask" class="metamask-logo" /> MetaMask Wallet
				</h1>
	...
	<div class="torus column">
		<Loader
			...
			let:load
			fromPromise={() =>
				loadTorusProvider().then((provider) => getAccountsFromProvider(provider, 'Torus'))}
			bind:result={$connectedProviderAccounts['Torus']}
			let:result={accounts}
			let:cancel
			whenCanceled={disconnectTorusProvider}
		>
			<svelte:fragment slot="header" let:status>
				{#if status !== 'resolved'}
					<div class="bar">
			...
			<div slot="idle" class="card">
				...
				<small
					>Supports: email address, Google, Facebook, Twitter, Discord, LINE, Apple ID, GitHub,
					Twitch, LinkedIn, WeChat, KakaoTalk</small
				>
			</div>

			<Portfolio name="Torus Wallet" provider={portfolioProvider} {accounts}>
	...
	<div class="portis column">
		<Loader
			...
			let:load
			fromPromise={() =>
				loadPortisProvider().then((provider) => getAccountsFromProvider(provider, 'Portis'))}
			bind:result={$connectedProviderAccounts['Portis']}
			let:result={accounts}
			let:cancel
			whenCanceled={disconnectPortisProvider}
		>
			<svelte:fragment slot="header" let:status>
				{#if status !== 'resolved'}
					<div class="bar">
			...
			<Portfolio name="Portis Wallet" provider={portfolioProvider} {accounts}>
	...
	<div class="idx column">
		<Loader
			...
			let:then={accounts}
			let:cancel
			whenCanceled={disconnect3IDConnect}
		>
			<svelte:fragment slot="header" let:status>
				{#if status !== 'resolved'}
					<div class="bar">
