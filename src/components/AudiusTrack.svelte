	...
	<div
		...
		title={[
			[
				track.title,
				track.user?.name &&
					`by ${track.user.name}${track.user?.handle && ` (@${track.user?.handle})`}`
			],
			[track.description],
			[
	...
	>
		<div class="columns">
			{#if track.artwork}
				<img
					class="track-artwork"
					srcset={Object.entries(track.artwork)
						.map(([size, src]) => `${src}`)
						.join(', ')}
					...
					<div class="row-inline">
						...
						<span class="row">
							...
							{#if track.release_date}
								<span class="released">
									<Date date={track.release_date} layout="horizontal" showTime={false} />
								</span>
							{/if}
						</span>
					</div>
		...
		<div class="player">
			<AudiusTrackPlayer trackId={track.id} isDownloadable={track.downloadable} bind:isPlaying />
		</div>
