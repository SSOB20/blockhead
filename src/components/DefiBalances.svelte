<script lang="ts">
	...
	$: if (defiProvider === 'Zapper')
		getAllApps().then(
			(_) => (allZapperAppConfigs = Object.fromEntries(_.map((app) => [app.id, app])))
		);
	...
	$: defiBalancesDescription =
		web3Apps?.map(({ name }) => name).join('/') || `${network.name} DeFi`;
...
{#if network && address}
	...
	{#if defiProvider === 'Zapper' || network.chainId !== 1}
		<Loader
			...
			then={(zapperDefiBalances) =>
				[...zapperDefiBalances.values()].filter(({ products }) => products.length)}
		...
		>
			...
			<div
				...
				class:scrollable-list={isScrollable &&
					zapperDefiBalances.flatMap(({ products }) => products).length > 6}
			>
				{#each zapperDefiBalances as { appId, products, meta }, i}
					...
					{#each products as { label, assets, meta: productMeta }, j (label)}
						<div
							...
							style={cardStyle(
								web3AppConfig?.colors ??
									(zapperAppConfig?.primaryColor ? [zapperAppConfig?.primaryColor] : [])
							)}
							transition:scale
						...
						>
							<div class="bar">
								<h5 class:card-annotation={computedLayout === 'horizontal-alternate'} class="row">
									...
									<span>
										<a href="/apps/{web3AppConfig?.slug ?? appId}/address/{address}"
											>{web3AppConfig?.name ?? label}</a
										>
							...
							<div class="defi-protocol-balances column">
								{#each assets as { key, type, appId, groupId, network: networkName, symbol, address, name, decimals, supply, tokens, pricePerShare, price, { liquidity, reserves, apy } = { }, { label, secondaryLabel, images, statsItems } = { }, balance, balanceRaw, balanceUSD }}
									...
									<div class="defi-protocol-balance card column">
										...
										<header
											class="bar"
											title="{formatKebabCase(groupId)}: {label} ({formatKebabCase(type)})"
										>
											<h6>
												{#if type === 'contract-position'}
													<AddressWithLabel {network} {address} {label} format="middle-truncated" />
										...
										{#if tokens?.length && (showUnderlyingAssets || type === 'contract-position')}
											<div class="underlying">
												{#each tokens as { id, type, appId, groupId, price, supply, network: networkName, symbol, name, address, decimals, tokens: innerTokens, { } = { }, { } = { }, pricePerShare, balance, balanceRaw, balanceUSD }}
													{@const network =
														networksByChainID[chainIdByNetworkName[networkName] ?? 1]}
	...
	{#if defiProvider === 'Zerion DeFi SDK' && network.chainId === 1}
		{#if provider}
			<Loader
				...
				fromPromise={() =>
					getDefiBalances({
						protocolNames: web3Apps?.flatMap(({ views }) =>
							views.flatMap(({ providers }) => providers?.zerionDefiSDK ?? [])
						),
						network,
						provider,
						address
