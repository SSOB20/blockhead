<script lang="ts">
	...
	export let fromStore:
		| (() => Readable<Result<LoaderResult>> | Promise<Readable<Result<LoaderResult>>>)
		| undefined;
	...
	export let fromUseInfiniteQuery:
		| UseInfiniteQueryStoreResult<LoaderResult[number], LoaderError>
		| undefined;

	export let then: (result: LoaderResult) => LoaderReturnResult = (result) =>
		result as unknown as LoaderReturnResult;
	...
	$: if (started) {
		...
		if (fromPromise) promise = fromPromise();

		if (fromStore) {
			const _store = fromStore();
			_store.then ? _store.then((_) => (store = _)) : (store = _store);
		}

		if (fromHoudiniQuery) houdiniQuery = fromHoudiniQuery();

		if (fromUseQuery) fromUseQuery.setEnabled(true);

		if (fromUseInfiniteQuery) fromUseInfiniteQuery.setEnabled(true);
	} else {
		if (fromUseQuery) fromUseQuery.setEnabled(false);

		if (fromUseInfiniteQuery) fromUseInfiniteQuery.setEnabled(false);
	}
	...
	function load() {
		...
		if (fromUseQuery) $fromUseQuery.refetch();

		if (fromUseInfiniteQuery) $fromUseInfiniteQuery.refetch();
	}

	async function cancel() {
		if (whenCanceled) await whenCanceled().catch(() => {}); // .catch(console.error)

		status = LoadingStatus.Idle;

		if (startImmediately) load();
	}
	...
	$: if (store?.subscribe) {
		if ($store.loading) {
			if ($store.data) result = then($store.data);
			status = LoadingStatus.Loading;
		} else if ($store.error) {
			error = $store.error;
			status = LoadingStatus.Errored;
...
{#if !isHidden}
		...
		>
			<svelte:fragment slot="header" let:isOpen let:toggle>
				...
				>
					...
					<button class="small" data-after={isOpen ? '⏶' : '⏷'} on:click={toggle}
						>{isOpen ? 'Hide' : 'Show'}</button
					>
				</slot>
			</svelte:fragment>
	...
	{:else}
		<slot name="header" {status} {loadingMessage} {errorMessage} {load} {cancel} />
		...
		{:else if layout === 'default'}
			<SizeContainer {containerClass} class="loader stack {contentClass}" {isOpen} {clip}>
				{#if status === LoadingStatus.Resolved || (fromStore && status === LoadingStatus.Loading && result)}
					<div class="column" transition:fade>
						<slot
							{result}
					...
					{#if status === LoadingStatus.Resolved || status === LoadingStatus.Errored}
						<div class="row">
							...
							{#if $fromUseInfiniteQuery.hasPreviousPage}
								<button class="small" on:click={() => $fromUseInfiniteQuery.fetchPreviousPage()}
									>Previous</button
								>
							{/if}

							{#if $fromUseInfiniteQuery.hasNextPage}
								<button class="small" on:click={() => $fromUseInfiniteQuery.fetchNextPage()}
									>Next</button
								>
							{/if}
						</div>
