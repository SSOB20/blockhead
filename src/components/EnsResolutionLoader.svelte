<script lang="ts">
	...
	const lookupAddress = memoizedAsync(async (address: Ethereum.Address) => {
		...
		throw new Error(
			`The address "${address}" doesn't reverse-resolve to an ENS name (or there's an issue with the${
				providerName === NetworkProvider.Default ? `` : ` ${providerName}`
			} JSON-RPC connection).`
		);
	...
	const resolveName = memoizedAsync(async (ensName: string) => {
		...
		throw new Error(
			`The ENS Name "${ensName}" doesn't resolve to an address (or there's an issue with the${
				providerName === NetworkProvider.Default ? `` : ` ${providerName}`
			} JSON-RPC connection).`
		);
	...
	export let showIf:
		| (({ address, ensName }: { address: Ethereum.Address; ensName: string }) => boolean | any)
		| undefined;
	...
	<Loader
		...
		fromUseQuery={address &&
			...
			useQuery({
				...
				queryFn: async () => await lookupAddress(address.toLowerCase())
			})}
		loadingIcon={ENSIcon}
		loadingIconName="ENS"
	...
	<Loader
		...
		fromUseQuery={ensName &&
			...
			useQuery({
				...
				queryFn: async () => await resolveName(ensName.toLowerCase())
			})}
		loadingIcon={ENSIcon}
		loadingIconName="ENS"
