	...
	<time class="date" datetime={dateObject.toISOString()}>
		<abbr class="layout-{layout}" title={dateObject.toISOString()}>
			...
			{#if format === 'absolute' || format === 'both'}
				<span class="absolute">
					{#if format === 'both' && layout === 'horizontal'}({/if}<span class="day"
						>{dateObject.toLocaleDateString()}</span
					>
					{#if showTime}<span class="time">{dateObject.toLocaleTimeString()}</span
						>{/if}{#if format === 'both' && layout === 'horizontal'}){/if}
				</span>
			{/if}
		</abbr>
	</time>
...
<style>
	.layout-vertical,
	.layout-vertical .absolute:only-child {
		display: flex;
