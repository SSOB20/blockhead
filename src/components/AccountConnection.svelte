<script lang="ts">
	...
	// Shared state
	$: if (state?.walletConnection?.walletType) walletType = state.walletConnection.walletType;
		...
		>
			...
			{#if state}
				...
				>
					<div class="wallet-icon-container stack">
						<Icon imageSources={[walletConfig?.icon]} />
						{#key state.chainId}{#if state.chainId}<div class="network-icon" transition:scale>
									<NetworkIcon network={networksByChainID[state.chainId]} />
								</div>{/if}{/key}
					</div>
					...
					<div class="column align-start">
						{#if state.address}
							<Address
								...
								network={networksByChainID[state.chainId]}
