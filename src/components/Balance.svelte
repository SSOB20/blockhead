<script lang="ts">
	...
	let erc20Contract;
	if (contractAddress) erc20Contract = new ethers.Contract(contractAddress, erc20ABI, provider);
...
<div class="stack">
	{#if provider && address && (symbol === 'ETH' || erc20Contract)}
		<Loader
			fromPromise={() =>
				symbol === 'ETH' ? provider.getBalance(address) : erc20Contract.balanceOf(address)}
