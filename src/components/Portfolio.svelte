...
<script lang="ts">
	...
	$: if (editable && name === '') state = State.Editing;
...
<section
	...
	on:keydown={(e) => {
		if (e.code === 'Escape') state = State.Idle;
	}}
...
>
	<header class="bar">
		...
		{#if editable}
			<div class="stack-inline">
					...
					<div class="bar align-end" transition:scale>
						{#if state === State.Idle}
							<button
								...
								transition:scale>Add Account</button
							>
						{/if}
						<InlineContainer containerClass="align-end" isOpen={accounts.length > 0}>
							<button data-before="✎" on:click={() => (state = State.Editing)} transition:scale
								>Edit</button
							>
						</InlineContainer>
					</div>
				...
				<InlineContainer containerClass="align-end" isOpen={state === State.Editing}>
					<div class="bar align-end" transition:scale>
						<button class="destructive" data-before="☒" on:click={() => dispatch('delete')}
	...
	{#if accounts}
		<SizeContainer
			containerClass="align-bottom"
			isOpen={state === State.Adding || !accounts.length}
			clip
		>
			<div class="stack">
				...
				{:else if !accounts.length}
					<div class="card" in:scale|local={{ start: 0.5 }} out:scale>
						...
						{#if editable}
							<p>
								You can <a on:click={() => (state = State.Adding)}
									>add a new wallet address manually</a
								>, or import an address by connecting a wallet service!
							</p>
						{/if}
					</div>
				{/if}
				...
				>
					{#if state === State.Editing}
						<div class="row edit-controls" transition:scale>
							<button class="destructive" data-before="☒" on:click={() => deleteAccount(i)}
	...
	<SizeContainer
		containerClass="sticky-bottom"
		isOpen={showOptions && accounts.length && state !== State.Editing}
	>
