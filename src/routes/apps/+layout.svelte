<script lang="ts">
	...
	$: if ($page.url.pathname.startsWith('/apps')) {
		$web3AppSlug =
			$page.params.web3AppSlug || $page.url.pathname.match(/^\/apps\/([^/]+)/)?.[1] || '';
...
<svelte:head>
	<title
		>{$addressOrEnsName ? `${$addressOrEnsName} | ` : ''}{$web3AppSlug && $web3AppConfig
			? `${$web3AppConfig.name} ${$currentView}`
			: `Apps`} | Blockhead</title
	>
</svelte:head>
					...
					{#if $web3AppSlug && $web3AppConfig}
						<span in:fly={{ y: 20, duration: 200 }} out:fly={{ y: -20, duration: 200 }}>
							<InlineContainer class="stack-inline">
								{#key $web3AppConfig}<mark
										in:fly={{ y: 20, duration: 200 }}
										out:fly={{ y: -20, duration: 200 }}>{$web3AppConfig.name}</mark
									>{/key}
							</InlineContainer>
							<InlineContainer class="stack-inline">
								{#key $currentView}<span
										in:fly={{ y: 20, duration: 200 }}
										out:fly={{ y: -20, duration: 200 }}>{$currentView}</span
									>{/key}
							</InlineContainer>
						</span>
					{:else}
			...
			<select bind:value={$web3AppSlug}>
				...
				{#each web3AppsBySection as { title, apps }}
					<optgroup label={title}>
						{#each apps as app}
							<option value={app.slug} style={`--primary-color: ${app.colors?.[0]}`}
								>{app.name}</option
							>
						{/each}
					</optgroup>
