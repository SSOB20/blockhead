...
export const networks: Ethereum.Network[] = [
	...
	},
	{
		...
		rpc: ['http://54.210.246.165:8545', 'http://3.86.104.251:8545', 'http://18.204.206.179:8545'],
		faucets: [],
		explorers: [],
	...
	},
	{
		...
		rpc: ['https://rpc.oasiseth.org:8545'],
		faucets: ['http://faucet.oasiseth.org'],
		explorers: [],
...
for (const network of networks) networksByChainID[network.chainId] = network;

export const networksBySlug: Record<string, Ethereum.Network> = {};
for (const network of networks) networksBySlug[network.slug] = network;

export const networksBySlip44: Record<number, Ethereum.Network> = {};
for (const network of networks) if (network.slip44) networksBySlip44[network.slip44] = network;

const testnetSlugsForMainnetSlugs = {
	ethereum: ['ethereum-kovan', 'ethereum-rinkeby', 'ethereum-ropsten', 'ethereum-goerli'],
	polygon: ['polygon-mumbai'],
	'arbitrum-one': ['arbitrum-rinkeby'],
	optimism: ['optimistic-goerli', 'optimistic-kovan'],
	...
	// 	'skale-testnet',
	// ],
	aurora: ['aurora-testnet', 'aurora-betanet']
	// 'nervos': [
	// 	'nervos-godwoken',
	// ],
};
...
export const testnetsForMainnets = Object.fromEntries<Ethereum.Network[]>(
	Object.entries(testnetSlugsForMainnetSlugs).map(([mainnetSlug, testnetSlugs]) => [
		mainnetSlug,
		testnetSlugs.map((slug) => networksBySlug[slug])
	])
);
...
export const evmL2Networks = ['aurora'].map((slug) => networksBySlug[slug]);

export const otherNetworks = networks.filter(
	(network) =>
