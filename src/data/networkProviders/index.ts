...
export const networkProviderConfigs: NetworkProviderConfig[] = [
	{
		...
		get: ({ network }) => new JsonRpcProvider(network.providers?.[0], network.chainId)
	},
...

	{
		...
		icon: InfuraIcon,
		get: ({ network }) =>
			new InfuraProvider(network.chainId, env.INFURA_PROJECT_ID, env.INFURA_PROJECT_SECRET)
	},
...

	{
		...
		get: ({ network, connectionType = NetworkProviderConnectionType.WebSocket }) =>
			new {
...

	{
		...
		get: ({ network }) => new EtherscanProvider(network.chainId, env.ETHERSCAN_API_KEY)
	},

	{
		...
		get: ({
			...
			const figmentProviderConfig = figmentProviderConfigs.find(
				(figmentProviderConfig) =>
			...
			return new {
				...
				`${
					figmentProviderConfig.connectionType === NetworkProviderConnectionType.WebSocket
						? 'wss'
						: 'https'
				}://${figmentProviderConfig.subdomain}.datahub.figment.io/apikey/${
					env.FIGMENT_DATA_HUB_APP_API_KEY
				}`,
				network.chainId
			);
		}
...

	{
		...
		get: ({ network, connectionType = NetworkProviderConnectionType.RPC }) =>
			new JsonRpcProvider(
...
export const networkProviderConfigByProvider = Object.fromEntries(
	networkProviderConfigs.map((networkProviderConfig) => [
		networkProviderConfig.provider,
		networkProviderConfig
	])
);

export const networkProviderConfigByNetworkSlug = Object.fromEntries(
	Object.entries({
		'arbitrum-one': [NetworkProvider.PocketNetwork, NetworkProvider.Figment],
		avalanche: [NetworkProvider.PocketNetwork, NetworkProvider.Figment],
		'avalanche-fuji': [NetworkProvider.Figment],
		bsc: [NetworkProvider.Figment],
		celo: [NetworkProvider.Figment],
		'celo-alfajores': [NetworkProvider.Figment],
		ethereum: [
		...
		fantom: [NetworkProvider.Figment],
		'harmony-shard0': [NetworkProvider.PocketNetwork],
		'harmony-shard1': [],
		'oasis-paratime': [NetworkProvider.Figment],
		polygon: [NetworkProvider.PocketNetwork, NetworkProvider.Figment, NetworkProvider.QuickNode],
		'polygon-mumbai': [NetworkProvider.Figment],
		optimism: [NetworkProvider.QuickNode]
		...
		[
			...
			...networkProviders.map((networkProvider) => networkProviderConfigByProvider[networkProvider])
		]
	])
);
...
export const getEthersProvider = memoizedAsync(
	async ({
		network,
		networkProvider,
		connectionType = NetworkProviderConnectionType.RPC,
